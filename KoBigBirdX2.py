...
def replace_html(my_str): # add more filters if needed  
...
for f in range(10):
    ...
    for i in tqdm(range(len(train_sentences)), desc="augmenting data kr-en-kr"):
    ...
    '''
    with open("backtranslated_sentences_dict_zh_cn.pkl", "rb") as pkl_dict:
        chn_aug_dict = pickle.load(pkl_dict) 
    
    for i in tqdm(range(len(train_sentences)), desc="augmenting data kr-cn-kr"):
        if train_sentences[i] in chn_aug_dict.keys():
                add_train_sentences.append(replace_html(chn_aug_dict[train_sentences[i]]))
                add_train_Y1.append(train_Y1[i]) 
                add_train_Y2.append(train_Y2[i]) 
                add_train_Y3.append(train_Y3[i]) 
                add_train_Y4.append(train_Y4[i]) 
    ...
    class NeuralCLF(nn.Module): 
        def __init__(self, plm="monologg/kobigbird-bert-base", type_classes=4, polarity_classes=3, tense_classes=3, certainty_classes=2): 
            ...
            self.weighted_layer_pooling = WeightedLayerPooling(self.config.num_hidden_layers, 9) 
        ...
        def forward(self, input_ids, attn_masks):
            ...
            x = self.mean_pooling(x, attn_masks) 
    ...
    train_sampler = RandomSampler(train_data) # shuffle 
    ...
    val_sampler = SequentialSampler(val_data) # do not shuffle validation data 
    ...
    criterion = {
        "type": type_loss_func, 
        "polarity": polarity_loss_func, 
        "tense": tense_loss_func, 
        "certainty": certainty_loss_func 
    } 
    ...
    for epoch_i in tqdm(range(0,epochs), desc="Epochs", position=0, leave=True, total=epochs): 
        ...
        if best_loss > avg_val_loss: 
            ...
            torch.save(model.state_dict(), f"KoBigBird_X2_augmented_Fold{f+1}_Epoch:{epoch_i+1}_loss:{best_loss}_f1:{f1_mult}.pt") 
